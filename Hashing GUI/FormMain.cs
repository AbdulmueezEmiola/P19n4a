...
namespace Hashing_GUI
{
    public partial class FormMain : Form
    {
        ...
        private void Setup()
        {
            ...
            dgvHashTable.Columns[2].Name = "З/п";
            dgvHashTable.Columns[2].Width = 100;
            dgvHashTable.Columns[3].Name = "ФИО сотрудника";
        ...
        public void PopulateDataGrid()
        {
            ...
            foreach (var tableCell in data)
            {
                if (tableCell != null)
                {
                    var (number, salary, employee) = tableCell.Info;
                    dgvHashTable.Rows.Add(i, number, salary, employee, tableCell.Next);
            ...
            if (FileUtils.ChooseFileToOpen(ref filename))
            {
                ...
                using (var reader = new StreamReader(File.OpenRead(filename)))
                {
                    ...
                    while (!reader.EndOfStream && result)
                    {
                        ...
                        result = EmployeeInfo.TryReadAsText(reader, out var tmp);
                        if (result)
                        {
                            _table.Add(tmp);
                        }
        ...
        {
            FormAddEmployeeInfo form = new FormAddEmployeeInfo(info =>
            {
                _table.Add(info);
                PopulateDataGrid();
            });
